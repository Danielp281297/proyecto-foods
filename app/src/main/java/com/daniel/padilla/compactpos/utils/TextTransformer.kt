package com.deoslv.bancaribe.utils
...
class PhoneNumberInputVisualTransformation(
    private val fixedCursorAtTheEnd: Boolean = true
) : VisualTransformation {
    private class FixedCursorOffsetMapping(
        private val contentLength: Int,
    ...
    override fun filter(text: AnnotatedString): TransformedText {
        ...
        val formattedNumber = if (inputText.length >= 3) {
            val firstThreeDigits = inputText.substring(0, 3)
            val remainingDigits = inputText.substring(3)
            "$firstThreeDigits-$remainingDigits"
        } else {
            inputText
        }
    ...
    // Resto del código de las clases FixedCursorOffsetMapping y MovableCursorOffsetMapping
    // ...
